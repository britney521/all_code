...
fieldnames = ['评论ID', '综合评分','商品型号','评论时间','评论内容','购买类型','有用投票数']
...
stars = {
    ...
    '904af42830e12750.png': 3,
    '7021b7b91e5b4ff3.png': 2,
}
# 100081225483
datas = pd.read_csv('data/diff.csv').iloc[:, 0].tolist()
datas = ['https://item.jd.com/{}.html'.format(item) for item in datas]

keywords = ['物流', '包装','快递', '价格','包裹', '客服服务','运输','送货','降价','活动价','保价','发货','态度','处理','退货','退款']
for index, row in enumerate(datas[10:]):
                ...
                for type in ['中评','差评']:
                    ...
                    for i in range(1,1000):
                        ...
                        for item in lists:
                            ...
                            if not any(x in safe_text for x in keywords):
                                continue
                            nickname = item.select_one('.jdc-pc-rate-card-nick').get_text().strip()
                            leftinfo = item.select_one('.jdc-pc-rate-card-info-left')
                            date = leftinfo.select_one('.date').get_text().strip()
                            starimg = item.find('img').get('src').split('/')[-1]
                            star = stars.get(starimg,5)
                            itemname = item.select_one('.info').get_text().strip()
                            likecount = item.select_one('.jdc-count').get_text().strip()
                            
                            
                            # if safe_text not in comments:
                            csvfile.writerow([nickname, star,itemname,date,safe_text,'已购',likecount])
